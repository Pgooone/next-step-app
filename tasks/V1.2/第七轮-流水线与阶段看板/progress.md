...
> 已 commit：`8cf8b16`（视觉定稿 / 决策 / 边界审计）、`6a3e504`（三件套 / 审查 / 交互精确化）。
> **下一步（vibe-coding 第 4 步·实现）= T1 承重墙 spike。**
>
> 权威设计 = 详细设计 §五任务表 + §四承重墙（`../../../docs/V1.2/第七轮-流水线与阶段看板/详细设计.md`）；本文是其进度镜像。

## 任务清单（T1~T8 · 依赖序 · 每卡门禁绿即单独 commit）

- [ ] **T1**（✅ 承重）承重墙 spike：A 冻结释槽 + B re-attach 续聊 —— AC-2/3/12 命门；lead 独立复跑两 spike；T1-B 不绿则 AC-12 降级须用户确认；探针收官删 → [`T1.md`](T1.md)
