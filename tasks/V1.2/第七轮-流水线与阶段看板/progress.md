...
> **T1 ✅ + T2 ✅ + T3 ✅ + T4 ✅ + T5 ✅ + T6 ✅ + T7 ✅ 收官（2026-06-27/28）。承重三卡(T1/T3/T6)全绿；T6 真浏览器 AC-8/9/11/12 + T7 真浏览器 AC-13（9 判据）全 PASS（全真实数据、pageErrors=0）+ 门禁 505。**T8 ✅ 收官——第七轮 T1~T8 全部完成、已 push 同步**（删探针 505→496 + 回填 8 个 ADR hash + registry 复位 + push 14 commit `f430d95`→`9716e13`）。下一步（独立于第七轮）= 处理「跨 run evict 误杀」（用户定先收官第七轮、之后处理；调查已证会话很轻、推荐走「改槽计量」方案 B）。⚠️ T6 揪出已知限制（D-R7-07 决策2）：跨 run 复用同 agent 时，新 run 完成阶段会 evict 误杀用户「进入完整对话」复活的同 agent 会话（pre-existing 缺陷、单 run AC-12 不受影响、留二期专卡、已向用户披露）。T2 flaky 测试已于 T5 期修（D-R7-06）。**
>
> 权威设计 = 详细设计 §五任务表 + §四承重墙（`../../../docs/V1.2/第七轮-流水线与阶段看板/详细设计.md`）；本文是其进度镜像。

## 任务清单（T1~T8 · 依赖序 · 每卡门禁绿即单独 commit）

- [x] **T1**（✅ 承重·**GO**）承重墙 spike：A 冻结释槽 + B re-attach 续聊 —— AC-2/3/12 命门**双命门确定性 GO**；ultracode 10-agent 对抗去风险(GO_WITH_MITIGATION) + agent team 实现 + lead 独立复跑 vitest 9/9 + 全量 435/435 + tsc 0 + eslint 0 + 变异 sanity（destroy 不删 Map→A1/A4 FAIL，证非 vacuous）；T1-B 全绿、AC-12 无须降级；**T8 已删两探针**（命门事实已迁 T3/T6 承重 verify + ADR D-R7-01） → [`T1.md`](T1.md)
...
- [x] **T8** ✅ 收尾 + 回写 docs + 残留清理（依赖全部）—— AC-15 达成；删 T1 两 spike 探针（505→496，命门已迁 T3/T6 承重 verify + ADR D-R7-01）+ 回填 ADR D-R7-01~08 全部 commit hash（f430d95/23f2460/636133e/75cb0b3/305d198/8bc224b/601954e/9716e13）+ 全量 registry 复位（备份 `~/.pi/projects.json.bak-r7-cleanup`）+ 门禁复跑（lint 0 / tsc 0 / test 496）+ push（14 commit `f430d95`→`9716e13` → origin/v1.2 + ff origin/master）+ ls-remote 实测同步 → [`T8.md`](T8.md)
...
- **探针去留**：两 spike 文件是开工门禁、不进长期测试集，**T8 已删**（命门事实已写入本契约 + ADR D-R7-01，已迁入 T3/T6 承重 verify；删后全量 vitest 505→496）。
