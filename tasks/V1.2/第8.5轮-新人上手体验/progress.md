...
> ✅ **T5（深色 shader 首页·试点）收官**（Phase A 字体 `6c061ce` + Phase B 视觉 `9ce6b74`；真浏览器唯一 oracle 双层验收、eyebrow 胶囊 D-V1.2-73；ADR D-R8.5-13）。**T1+T5 均收官 → 第 8.5 轮全轮完成。**
...
| **T5** | 炫酷深色首页（shader 组件 + 深色玻璃首页 + 字体自托管 woff2〔**+迁 layout 旧 google mono**〕 + 性能/降级护栏 + 试点开关 + **真浏览器作 build oracle**） | medium · 试点 | ✅ **收官**（Phase A 字体 `6c061ce`〔9 woff2 自托管 + 迁 layout mono、lead 亲跑编译 gate〕 + Phase B 视觉 `9ce6b74`〔原生 WebGL shader 零 three + 深色玻璃 + 护栏四条 + 试点开关 NEXT_PUBLIC_HOME_SHADER + eyebrow 胶囊 D-V1.2-73〕；真浏览器唯一 oracle 双层验收 pageErrors=0/nodeFs=0、门禁 tsc0/lint0/test517；ADR D-R8.5-13） |
...
4. ⏸ **续做（下次）**：
   - ✅ **T5 深色 shader 首页·试点 已收官**（2026-06-30：Phase A `6c061ce` + Phase B `9ce6b74`，真浏览器双层验收、eyebrow 胶囊 D-V1.2-73、ADR D-R8.5-13）。**第 8.5 轮全部卡收官。**
