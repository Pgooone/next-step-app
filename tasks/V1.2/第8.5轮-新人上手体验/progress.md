...
> ⏸ **T5（深色 shader 首页·试点）+ T1（首用引导 Tour·批次2）暂停待续**（用户拍板 D-V1.2-70：批次1 收官即 push、T5 暂停；T1 批次2 本次未起）。
...
| 卡 | 内容 | 量级 | 状态 |
|---|---|---|---|
| **T1** | 首用引导 Tour（方案 B 分层 · overlay 引擎 + 总览5/深度6 + **两模态加 `initial*` 承重改造〔blocker1〕** + **深度轨空环境降级〔blocker2〕** + seen 持久化 + 新手引导按钮 + mini-spike + ADR） | medium→偏大 · tentpole · **批次2** | ⏸ **暂停待续**（批次2 本次未起；spec 已审计订正就绪，续做先做 §0 承重改造 + mini-spike，读卡 T1 + 前端设计 §1 订正） |
...
4. ⏸ **续做（下次）**：
   ...
   - **T1 首用引导 Tour·批次2**——先做 §0 两模态 `initial*` 承重改造 + mini-spike，再铺总览5/深度6；深度轨空环境降级（D-R8.5-03）。
