...
- [x] ✅ **AC④ 完整 onboarding 真浏览器闭环 —— 完成（2026-06-20 lead 亲自补跑 PASS）**。`scripts/r5-onboarding-e2e.mjs`（拷 /tmp/pw 跑解析 playwright、自管两次 dev 起停）：UI 建 doc agent → 起会话 create_artifact 落盘 → **重启 dev** → `?session=` re-attach → UI 发 propose_edit 落 PendingChange（**核心：re-attach 后 doc 工具集未丢**）→ PendingChangeCard「全部 ✓」二次确认 → 物化 v1→v2（正文 goodbye world / pending 清空）。4 断言全 PASS、无 `pageerror`（console 仅文件 API 403 + dev 重启窗口 ERR_CONNECTION_REFUSED/webpack-hmr，均非应用缺陷）、全程无 OOM。**修跑通真因**：脚本「全部 ✓」漏点 BUG-04 二次确认第二步「确认执行」（点一次只 `setConfirmAll` 弹确认条、不物化）→已补；另脚本注释「从项目根跑」错，须拷 /tmp/pw 才能 resolve playwright。
- [x] 门禁全绿（lint+test+tsc 358）+ 零回归 + 回写文档/memory ✅；真浏览器 AC④ 已补跑 PASS（lead 亲验截图 v2/goodbye world/「已确认全部 1 处」toast）
