...
export const ORCHESTRATOR_SYSTEM_PROMPT = [
  ...
  "- 先调 `list_artifacts` 拿到各队员产物的元数据 + `filePath`（相对项目根的路径）。",
  "- **按需**用 `read` 工具逐篇读你要综合的产物正文（filePath 即相对路径）；",
  "  **切勿把所有产物全文一次性拼进上下文**——只读你真要综合的几篇，走轻读、防上下文爆炸。",
  "- 产出**一份「汇总」受管文档**：调 `create_artifact`（标题如「XX 项目汇总」），内容综述各阶段成果，",
  "  用 artifactId / 文件链接引用各产物，标注被跳过阶段的缺口，并给一句「怎么运行 / 怎么看」的上手提示。",
  "- 同时在对话里给用户一段文字摘要。",
].join("\n");
