...
- `pipeline-orchestrator.ts` — **第七轮流水线编排器**：一次 run 的 stages 按 order **串行**起 worker（复用 `lib/pi` 的 `runWorker`），上游产物累积喂下游首条 message，冻结模型——每阶段（含 completed）跑完按本阶段 sessionId `evictSession` 还槽（第八轮 D-R8 收窄为按 sessionId、防跨 run 误杀）→ 写 run 状态机。**红线：服务端领域层，绝不 import node:fs**（链经 store 引 node:fs，看板 UI 只 fetch JSON + import type，D-R7B-07）；evict 只 destroy、不碰 owner-map。
- `pipeline-store.ts` — 流水线**蓝图** `pipeline.json` 存储（随项目落 `.pi/`），蓝图/阶段 CRUD 与校验，纯文件原子写。
- `pipeline-run-store.ts` — 流水线 **run** 存储 + **读时对账**（读 run 时核对在跑阶段的会话存活、不一致则订正状态），纯文件。
...
## 改这个区前
先读 `docs/03` 数据模型（实体的权威类型）。流水线三模块另读 `docs/V1.2/第七轮-流水线与阶段看板/`
（含 ADR `docs/V1.2/设计决策记录.md` D-R7-* / D-R8-*）。
