 ...
 *        ① evict 每阶段调一次（顺序 + agentId）② run 结束 fauxMap.size===0 ③ peak<=1
...
describe("承重 verify：runPipeline 每阶段 evict 后活会话计数确定性回落（F16/AC-2/3）", () => {
  /**
   * 唯一计数源 fauxMap + 按 agentId 真反查 ownerByAgent。三处闭包（faux worker 的 destroy 删除、
   * 真实 acquireSlot 的 activeCount、真实 evictAgentSessions 的 getSession）**必须引用同一 fauxMap**。
   *
   ...
   */
  function makeHarness(opts?: { destroyDeletesMap?: boolean; failFirstStage?: boolean }) {
    ...
    // faux runWorker：进阶段时 fauxMap.size 应为 0（上阶段已 evict），set worker 后留在 Map 模拟 completed
    // 不自销毁（坐实只有 evict 才释）；记 peak。第 1 阶段 failFirstStage 时先 set 再 throw（catch 路径）。
    // ★ownerByAgent 必须按**真实 agentId**（args.profile.id，= runPipeline 传给 evict 的 stage.agentId）键，
    //   evict 才反查得到、destroy 同一 Map（否则 size 永不回落 = 假绿）。
    const fauxRunWorker = (async (args: { firstMessage: string; profile: { id: string } }) => {
      const idx = stageSeq++;
      const sid = "s" + idx;
      const agentId = args.profile.id;
      // 进阶段断言：上阶段已 evict 释槽 + acquireSlot 才放行 → size===0
    ...
    // 真实 acquireSlot，数同一 fauxMap；外层 spy 记 log["acq"+idx]。
    ...
    // 真实 evictAgentSessions，删同一 fauxMap；外层 spy 记 log["evict"+(stageSeq-1)] + 断言调用。
  ...
  it("①②③④ 连跑 4 阶段：evict 每阶段一次(顺序+agentId) + 结束 size===0 + peak<=1 + 顺序 acq→set→evict", async () => {
    ...
    // ① evict 每阶段一次，第二参 === 本阶段 agentId（顺序断言）
    expect(h.evictSpy).toHaveBeenCalledTimes(4);
    expect(h.evictSpy).toHaveBeenNthCalledWith(1, projectRoot(), agentIds[0]);
    expect(h.evictSpy).toHaveBeenNthCalledWith(2, projectRoot(), agentIds[1]);
    expect(h.evictSpy).toHaveBeenNthCalledWith(3, projectRoot(), agentIds[2]);
    expect(h.evictSpy).toHaveBeenNthCalledWith(4, projectRoot(), agentIds[3]);
  ...
  it("⑥ catch 路径释槽：runWorker 第 1 阶段已 set 后 throw → run failed + 该阶段 evict + size 回落 + 后续不跑", async () => {
    const { run, agentIds } = makeRun(4);
    ...
    // catch 内 best-effort evict 被调（释槽）：恰 1 次、命中第 1 阶段 agentId
    expect(h.evictSpy).toHaveBeenCalledTimes(1);
    expect(h.evictSpy).toHaveBeenNthCalledWith(1, projectRoot(), agentIds[0]);
    // size 回落（catch 内 evict 真删了 Map）
    expect(h.fauxMap.size).toBe(0);
...
describe("runPipeline AC-5 排队态 + 不限超时", () => {
  it("每阶段 acquireSlot 都传 {timeoutMs:Infinity}，且 statusDetail='queued' 在 acquireSlot 之前落盘", async () => {
    ...
    const result = await runPipeline(run, {
      ...
      acquireSlot: acquireSpy as unknown as typeof acquireSlot,
      setOwner: vi.fn(),
      evictAgentSessions: vi.fn(async () => []) as unknown as typeof evictAgentSessions,
      registerInnerSession: throwingRegister,
    });
...
describe("runPipeline 失败分支（仿 orchestrator.test.ts:378-404）", () => {
  async function runWithFirstResult(
    ...
    const result = await runPipeline(run, {
      ...
      acquireSlot: (async () => {}) as unknown as typeof acquireSlot,
      setOwner: vi.fn(),
      evictAgentSessions: evict as unknown as typeof evictAgentSessions,
      registerInnerSession: throwingRegister,
    });
