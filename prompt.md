...
1. 读 `tasks/progress.md` 决定开发顺序；
2. 按模块依赖批次，用 Agent 工具为每个模块派**子 Agent**（每个子 Agent 只做一个模块的实现 + 测试，上下文天然短）；
3. 每批全部完成后跑质量门禁，全绿才进下一批；
4. 全程更新 `tasks/progress.md` 与各模块 `tasks/<Mx>.md`。

真相源：`docs/需求文档.md`（要什么）、`docs/概要设计.md`（模块划分）、`docs/详细设计.md`（怎么做）。
...
1. 取当前批次未完成模块 → 各派一个**子 Agent**（附：该模块 `tasks/<Mx>.md` + 详细设计对应小节 + 相关现有代码位置）。
...
- 开工前先与用户对一遍 `docs/详细设计.md` 末尾的 **3 条待确认 lead 机制决策**（M5 存盘位置 / M6 单页分流 / M8 转交载荷）。
