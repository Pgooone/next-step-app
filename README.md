...
源自 `docs/需求文档.md`（由 `docs/Next-step改进原始需求.txt` 经多轮问答确认而来，全程见 `docs/QA/`）。共 **1 bug + 5 功能 + 1 外观**：

| 编号 | 一句话 | 状态 / 决策 |
|---|---|---|
| 功能#1 | 前端界面深度解析报告 | ✅ 已交付 `docs/前端界面深度解析报告.md` |
...
- `docs/需求文档.md` —— ★ 正式 V1.1 需求文档（现状→决策→落地→验收）
- `docs/前端界面深度解析报告.md` —— 前端逐控件深度解析（约 26000 字）
- `docs/概要设计.md` —— 模块划分（vibe-coding 第 2 步）
- `docs/详细设计.md` —— 各模块详细设计
- `docs/设计决策记录.md` —— lead 机制决策（ADR：可选项 / 选定 / 对照北极星的理由，D-V1.1-01~04）
- `docs/文档评审报告.md` —— 4-reviewer agent-team 评审报告（R1~R17 已回改）
- `docs/后续迭代清单.md` —— 本期先不做、留作后续的项（@转交续接旧会话、功能#4 档2 等）
